-- Migration 034: Add idempotency_key to sales and update complete_sale_transaction for safe offline replay

-- 1. Add idempotency_key column to sales table
ALTER TABLE public.sales ADD COLUMN IF NOT EXISTS idempotency_key UUID;

-- 2. Create unique index on idempotency_key (only when not null)
CREATE UNIQUE INDEX IF NOT EXISTS sales_idempotency_key_idx ON public.sales (idempotency_key) WHERE idempotency_key IS NOT NULL;

-- 3. Update complete_sale_transaction to accept p_idempotency_key and short-circuit if already processed
...
BEGIN
  ...
  -- 1. Idempotency Check: if idempotency key is provided and already recorded for this shop, return existing sale immediately
