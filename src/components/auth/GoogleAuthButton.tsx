...
export function GoogleAuthButton({ loading = false, onClick, className, label, disabled = false }: GoogleAuthButtonProps) {
  return (
    <Button
      type="button"
      variant="outline"
      className={cn("h-12 w-full rounded-2xl border-border/70 bg-background/80 text-foreground", className)}
      onClick={onClick}
