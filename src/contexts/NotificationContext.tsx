...
export function NotificationProvider({ children }: { children: ReactNode }) {
  ...
  const permission: NotificationPermission | "unsupported" = supportsNativeNotifications
  ...
  useEffect(() => {
    ...
    const unreadNewItems = notifications.filter(
      (entry) => !entry.read_at && !seenIdsRef.current.has(entry.id),
    );
